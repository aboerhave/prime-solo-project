...
router.get('/off', (req, res) => {
    ...
    if (req.isAuthenticated() === false) { // unauthenticated
        console.log('forbidden/unauthenticated for getting off favorites');
        res.sendStatus(403);
    }
    else {  // authenticated
        let queryText = `select * from favorites 
...
router.put('/:attractionId', (req, res) => {
    ...
    if (req.isAuthenticated() === false) { // unauthenticated
        console.log('forbidden/unauthenticated for toggling favorite');
        res.sendStatus(403);
    }
    else {  // authenticated
...
router.post('/:attractionId', (req, res) => {
    ...
    if (req.isAuthenticated() === false) { // unauthenticated
        console.log('forbidden/unauthenticated for posting favorite');
        res.sendStatus(403);
    }
    else {  // authenticated
